            ...
            <div class="row breadcrumb-bar">
                ...
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">编辑骑手</li>
                    </ol>
                </div>
            </div>
        </div>
                    ...
                    <div class="widget-area-2 lochana-box-shadow">
                        ...
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 当前表的字段 -->
                                    ...
                                    <div class="form-group col-md-6 qishouNameDiv">
                                        <label>骑手姓名</label>
                                        <input style="width: 450px" id="qishouName" name="qishouName" class="form-control"
                                               placeholder="骑手姓名">
                                    </div>
                                    <div class="form-group col-md-6 qishouPhotoDiv">
                                        <label>头像</label>
                                        <img id="qishouPhotoImg" src="" width="100" height="100">
                                        <input name="file" type="file" id="qishouPhotoupload"
                                               class="form-control-file">
                                        <input name="qishouPhoto" id="qishouPhoto-input" type="hidden">
                                    </div>
                                    <div class="form-group col-md-6 qishouPhoneDiv">
                                        <label>骑手手机号</label>
                                        <input style="width: 450px" id="qishouPhone" name="qishouPhone" class="form-control"
                                               onchange="qishouPhoneChickValue(this)"  placeholder="骑手手机号">
                                    </div>
                                    <div class="form-group col-md-6 qishouIdNumberDiv">
                                        <label>骑手身份证号</label>
                                        <input style="width: 450px" id="qishouIdNumber" name="qishouIdNumber" class="form-control"
                                               onchange="qishouIdNumberChickValue(this)"  placeholder="骑手身份证号">
                                    </div>
...
<script>
    ...
    function upload() {
        ...
        <!-- 当前表的文件上传 -->
        $('#qishouPhotoupload').fileupload({
            url: baseUrl + 'file/upload',
            headers: {token: window.sessionStorage.getItem("token")},
            dataType: 'json',
            done: function (e, data) {
                var photoUrl= baseUrl + 'file/download?fileName=' + data.result.file;
                document.getElementById('qishouPhotoImg').setAttribute('src',photoUrl);
                document.getElementById('qishouPhoto-input').setAttribute('value',photoUrl);
            }
        });
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            ...
            httpJson("qishou/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
                    window.sessionStorage.removeItem('addqishou');
    ...
    //数字检查
        <!-- 当前表的数字检查 -->
        function qishouPhoneChickValue(e){
        ...
        function qishouIdNumberChickValue(e){
            var this_val = e.value || 0;
        ...
        function qishouDeleteChickValue(e){
            var this_val = e.value || 0;
            var reg=/^[1-9][0-9]*$/;
            if(!reg.test(this_val)){
                e.value = "";
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                ...
                password: "required",
                qishouName: "required",
                qishouPhoto: "required",
                qishouPhone: "required",
                qishouIdNumber: "required",
                sexTypes: "required",
            },
            messages: {
                username: "账户不能为空",
                ...
                qishouName: "骑手姓名不能为空",
                qishouPhoto: "头像不能为空",
                qishouPhone: "骑手手机号不能为空",
                qishouIdNumber: "骑手身份证号不能为空",
                sexTypes: "性别不能为空",
            }
        }).form();
    }
    ...
    // 获取当前详情
    function getDetails() {
        var addqishou = window.sessionStorage.getItem("addqishou");
        if (addqishou != null && addqishou != "" && addqishou != "null") {
            //注册表单验证
            $(validform());
            ...
            window.sessionStorage.removeItem('updateId');
            http("qishou/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
